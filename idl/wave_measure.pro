...
track_saus_test1,data=(-1.*smooth(ts_in,3)),errors=ts_er,fits,threads,/patch
...
x=findgen(N_ELEMENTS(tube))

check=''

WHILE check NE 'y' DO BEGIN
set_plot,'ps'
device,/encapsul,/color,filename='/Users/krishnamooroogen/Documents/PHYSICS/PhD/intpeak.eps'
plot,x*time,tube*Mm,xtitle='Time(s)',ytitle='Displacment(Mm)',xstyle=1,xthick=2,ythick=2,thick=2
errplot,x*time,(tube+pos_er)*Mm,(tube-pos_er)*Mm,thick=2
oplot,x*time,mysin(x,sin_pos[0:4])*Mm,linestyle=1,thick=1.3

print,'Where would you like to cut?'
READ,cutst,PROMPT='Start of cut? '
READ,cuten,PROMPT='End of cut? '

IF cutst lt cuten THEN BEGIN

cutst=divide(cutst,time)
cuten=divide(cuten,time)

res=poly_fit(x[cutst:cuten]*time,tube[cutst:cuten]*Mm,2,measure_errors=pos_er[cutst:cuten]*Mm,chisq=chi,yfit=fit,sigma=sigma)
...
loadct,2,/silent
oplot,x[cutst:cuten]*time,fit,linestyle=2,thick=2,color=100
...
plots,x_cen,y_cen,psym=7,color=100,SYMSIZE=2,thick=3
loadct,0,/silent
ENDIF ELSE BEGIN
check='n'
ENDELSE
...
plot,x*time,ts_vel_sum,xtitle='Time(s)',ytitle='Velocity(m/s)',xstyle=1
errplot,x*time,ts_vel_sum-tsv_er,ts_vel_sum+tsv_er
...
plots,x_cen,y_cen,psym=7,color=100,SYMSIZE=2
loadct,0,/silent
...
int_info={tube_list:(int_tube_list[*,1:*]*Mm),int_tube_error: int_terror[*,1:*]*Mm, int_time:int_time[1:*], $
int_time_pos:int_time_pos[1:*],time_xer:t_xer[1:*],time_yer:t_yer[1:*]}

vel_info={vel_tube_list:vel_tube_list[*,1:*],vel_tube_error:vel_terror[*,1:*], vel_time:vel_time[1:*], $
vel_time_pos:vel_time_pos[1:*], amp:vel_amp[1:*], period:vel_per[1:*],vel_time_xer:vel_time_xer[1:*],vel_time_yer:vel_time_yer[1:*]}


;tl=reverse(int_tube_list[*,1:*]*Mm)
;tler=reverse(int_terror[*,1:*]*Mm)
;plot,findgen()*time,tl(*,0),xstyle=1,xtitle='Time(s)',ytitle='Displacement(Mm)'
;ereplot,findegen()*time,tl(*,0)+,tl(*,0)-
;FOR i=1,8 DO BEGIN
