...
pro FOLLOW_THREAD_TESTING, min_tlen=min_tlen, area=area, out=out
...
posi = fltarr(nt)
pos_err = fltarr(nt)
flag_bins = intarr(nt) ;indicates invalid values (-1), data gaps (0), and found peaks (1 or 2)

val = fix(area)/2 ; old --> area-2

found_first_thread = 0 ;will be set to 1 ('True') once the first valid thread is found and saved to the output array
FOR j=0L, nt-(area+1) DO BEGIN ;Search over time
    FOR i=area, nx-(area+1) DO BEGIN ; Search over x
        ...
        IF image[i,j] GT mini THEN BEGIN
            ...
            ;reset temp values before finding thread
            tlen = 0.0
            posi[0:nt-1] = 0.0
            pos_err[0:nt-1] = 0.0
            posi[0:j] = -1.0
			...
			WHILE max(a) GT mini DO BEGIN
                ...
                IF b[0] LT 0 THEN BEGIN
                    ;if no other points found, then set 'a' to minimum value so loop is broken
                    a = mini
                ENDIF ELSE BEGIN
            ...
            IF tlen GT min_tlen THEN BEGIN
                ...
                IF n_elements(where(posi[*] gt 0.0)) GT 2.0 THEN BEGIN
