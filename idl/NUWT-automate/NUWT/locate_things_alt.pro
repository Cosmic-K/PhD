...
;   meas_size - sets number of pixels used in Gaussian fit, default value is 7,
...
pro LOCATE_THINGS_ALT, data, errors=errors, grad=grad, num_search_bins=num_search_bins, meas_size=meas_size, $
...
FOR p=0L, num_peaks_to_test-1 DO BEGIN
    ...
    IF (grad_left[i,j] GT grad) AND (grad_right[i,j] LT (-1.)*grad) THEN BEGIN
        ...
        IF KEYWORD_SET(weighted_mean) THEN BEGIN
            ;find sub-pixel location using weighted mean method
            sum_itensities = float(total(image[i-len:i+len]))
            weights = image[i-len:i+len]/sum_itensities
            wg_mean = total(weights*in2)/total(weights)
            wg_stddev = total(weights*(in2-wg_mean)^2)/total(weights)
            ;creating fake fit results (for quick and dirty saving of values)
            chisq = 0.0
            chisq_red = 0.0
            ; coeff = [1.0, wg_mean, 1.0, 0.0, 0.0]
            coeff = [1.0, wg_mean, wg_stddev, 0.0, 0.0]
            ; sigma = [0.0, wg_stddev, 0.0, 0.0, 0.0]
