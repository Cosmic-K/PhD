...
IF KEYWORD_SET(plot_peaks) OR KEYWORD_SET(plot_threads) THEN BEGIN
    ...
    IF KEYWORD_SET(underplot_td) THEN BEGIN
        plt_pk = IMAGE(slit_data[*,*], axis_style=0, position=[0.125, 0.10, 0.995, 0.92], /current)
    ENDIF ELSE BEGIN
...
IF KEYWORD_SET(plot_threads) THEN BEGIN
    ...
    IF KEYWORD_SET(underplot_td) THEN BEGIN
        plt_th_im = IMAGE(slit_data[*,*], axis_style=0, position=[0.125, 0.10, 0.995, 0.92], /current)
    ENDIF ELSE BEGIN
