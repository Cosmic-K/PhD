...
;
;OPTIONAL INPUTS:
;   res - spatial resolution of data (in [arcsec]). Defaults to a value of 1
;   cad - temporal cadence of the data (in [s]). Defaults to a value of 1
;   km_per_arcsec - a ratio of [km]/[arcsec]. Defaults to 725.27 which is the
...
;                                & 'fft_stats', added calculation of GOF test stats,
...
PRO WAVE_FROM_FFT, res=res, cad=cad, km_per_arcsec=km_per_arcsec, detrend=detrend,$
                   ...
                   min_freq_cutoff=min_freq_cutoff, min_cyc_cutoff=min_cyc_cutoff, $
...
pow_output_units = amp_output_units+'^{2}/'+freq_output_units
...
FOR h=0L, (n_threads-1) DO BEGIN
    ...
    IF KEYWORD_SET(iterate_padding) THEN BEGIN
        ...
        target_peak_bin = 20
        guess_freq = old_fft_stats[h].peak_freq[0]*freq_dt ; [Hz]
    ...
    IF (n_elements(where(tpos gt 0.0))) GT 2.0 THEN BEGIN
        ...
        IF (n_elements(where(tpos EQ 0.0))) LT 0.5*(n_elements(where(tpos GE 0.))) THEN BEGIN
            ...
            IF t_start ge 0 THEN BEGIN
                ...
                tpos = tpos[t_start:t_end];*dx
                terr = terr[t_start:t_end];*dx
                ...
                IF keyword_set(detrend) THEN BEGIN
                    ...
                    ; line_fit_result = poly_fit(findgen(t_end-t_start+1), tpos, 1, yfit=trend) ;fit linear trend line
                ENDIF ELSE BEGIN
                    trend = fltarr(len_thread) + mean(tpos) ;creates an array with the mean of the data
                    ; trend = fltarr(t_end-t_start+1) + mean(tpos) ;creates an array with the mean of the data
                ENDELSE
                ...
                ; IF ((moment(pow))[1]) GT 1e-9 THEN BEGIN ;OLD threashold that does not work well for large fft padding
                    ...
                    result_th_fft.power = pow*power_correction*(amp_dx^2)
                    result_th_fft.amplitude = 2.0*sqrt(pow/2.0)*amp_correction*amp_dx
           			result_th_fft.freq = f*freq_dt
                    result_th_fft.phase = phase
                    stats_th_fft.cpg = CPG

                    IF KEYWORD_SET(bootstrap) THEN BEGIN
                        result_th_fft.err_power = pow_err*power_correction*(amp_dx^2)
                        result_th_fft.err_amplitude = 2.0*sqrt(pow_err/2.0)*amp_correction*amp_dx
                    ...
                    ;Significant tests for power spectra
         			...
         			ENDIF ELSE BEGIN
                        ...
                        sig_vals = SIGNIF_NOISE_SPEC((tpos-trend), signif_levels, n_elements(f), color='white')
                        result_th_fft.signif_vals = sig_vals*signif_correction*(amp_dx^2)
                        loc_signif_pow = where(result_th_fft.power GT result_th_fft.signif_vals, /NULL)
                        ...
                        ; result_th_fft.signif_vals = sig_vals*power_correction
                    ...
                    IF stats_th_fft.num_signif_peaks GT 0.0 THEN BEGIN
                        ...
                        FOR jj=0, stats_th_fft.num_signif_peaks-1 DO BEGIN
                            ...
                            IF (sav_p LT 4) AND (f[bin_of_peak] GT min_freq_cutoff) THEN BEGIN
                                ...
                                stats_th_fft.peak_power[sav_p] = result_th_fft.power[bin_of_peak]
                                stats_th_fft.peak_amplitude[sav_p] = result_th_fft.amplitude[bin_of_peak]
                                stats_th_fft.peak_freq[sav_p] = result_th_fft.freq[bin_of_peak]
                                stats_th_fft.peak_vel_amp[sav_p] = 2*!PI*stats_th_fft.peak_power[sav_p]*stats_th_fft.peak_freq[sav_p]
                                stats_th_fft.peak_phase[sav_p] = phase[bin_of_peak]
                                stats_th_fft.peak_bin[sav_p] = bin_of_peak
                                IF KEYWORD_SET(bootstrap) THEN BEGIN
                                    stats_th_fft.err_peak_power[sav_p] = result_th_fft.err_power[bin_of_peak]
                                    stats_th_fft.err_peak_amplitude[sav_p] = result_th_fft.err_amplitude[bin_of_peak]
                                    stats_th_fft.err_peak_phase[sav_p] = phase_err[bin_of_peak]
                                ENDIF
                    ...
                    stats_th_fft.num_saved_waves = sav_p
                    IF stats_th_fft.num_signif_peaks GT sav_p THEN stats_th_fft.num_signif_peaks = sav_p
