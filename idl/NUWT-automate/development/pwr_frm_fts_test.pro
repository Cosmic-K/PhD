...
FOR k=0, nslits-1 DO BEGIN
    ...
    ;Initialize array to contain the results (not used at the moment)
    fft_results = {power:fltarr(nf, n_threads), displacement:fltarr(nf, n_threads), freq:fltarr(nf, n_threads), $
                   duration_series:fltarr(nf, n_threads), phase2:fltarr(nf, n_threads)}
    ...
    FOR j=0, (n_threads-1) DO BEGIN
        ...
        tpos = threads[j].pos ;dummy array with postions of the selected thread
        ...
        IF (n_elements(where(tpos gt 0.))) GT 2. THEN BEGIN

            ;skips enteries where half the data points are set to zero, i.e. no
            ;value was obtained at fitting stage.
            IF (n_elements(where(tpos EQ 0.))) LT 0.5*(n_elements(where(tpos GE 0.))) THEN BEGIN
                ...
                IF t gt 0 THEN BEGIN
                    ...
                    IF keyword_set(ctrend) THEN res = poly_fit(findgen(t1-t+1),tpos,1,yfit=trend) $ 
                        ELSE trend = mean(tpos)  ;calculates mean of data
            	    ...
            	    ;Doesn't let straight lines through
           		    IF ((moment(pow))[1]) GT 1e-9 THEN BEGIN
                        ...
                        ;SOMEWHAT COMPLICATED SECTION THAT FINDS PEAKS IN THE SIGNIFICANT POWER RESULTS
             			CONSEC, sig_vals, lo, hi, num

             			nsv=n_elements(sig_vals)
             			totcons=total(hi-lo)+num ;total number of sig values in consecuative groups
                         
             			IF (nsv GT 0) AND (sig_vals[0] ne -1) THEN BEGIN

                		    IF nsv EQ totcons THEN BEGIN 
                                ;all significant values are part a consecuative group (i.e. none are isolated)
                            ...
                            ENDIF ELSE BEGIN
                    		    IF totcons EQ 0 THEN BEGIN
                                    ;all significant values are isolated
                   				...
                   				ENDIF ELSE BEGIN 
                                    ;mixture of consecuative groups and isolated significant values
                       				FOR ii=0,num-1 DO BEGIN
                           			    bran=sig_vals[lo[ii]] & tran=sig_vals[hi[ii]]
                           				m=max(pow[bran:tran],loc)
