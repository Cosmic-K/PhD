...
FUNCTION SIGNIF_NOISE_SPEC, data_in, p, num_FFT, color=color, AR_coeff=AR_coeff
...
IF color EQ 'red' THEN BEGIN
    ...
    ;data_i_plus_1 = SHIFT(data_in, -1)
    ;data_i_plus_2 = SHIFT(data_in, -2)
    ;calculate the lag-1 (a1) and lag-2 (a2) coeffients
    ;a1 = 1.0/((N - 1)*s^2)*total(((data_in - mu)*(data_i_plus_1 - mu))[0:-2])
    ;a2 = 1.0/((N - 2)*s^2)*total(((data_in - mu)*(data_i_plus_2 - mu))[0:-3])
    a1 = (1.0/total(mean_diff^2))*total(mean_diff[0:-2]*mean_diff[1:*])
    a2 = (1.0/total(mean_diff^2))*total(mean_diff[0:-3]*mean_diff[2:*])
