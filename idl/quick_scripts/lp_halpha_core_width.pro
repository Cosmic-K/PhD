...
; average wing intensities at delta lambda = 900 mAA
 ...
 if minp gt 1 and minp lt nl-2 then begin
     m0=min(abs(ll +0.9), l0)
     m1=min(abs(ll -0.9), l1)
     if m0 lt 0.001 and m1 lt 0.001 then begin
         avgw=mean([sp[l0], sp[l1]])
         if show then begin
     ...
     endelse 
     ll0=interpol(ll[0:minp-1], sp[0:minp-1], (avgw-spmin)/2.+spmin)
     ll1=interpol(ll[minp+1:*], sp[minp+1:*], (avgw-spmin)/2.+spmin)
     corewidth=ll1-ll0
